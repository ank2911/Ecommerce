<template>
  <v-container class="search-page">
    ...
    <v-card v-for="items in searchItem.Search">
      ...
      <v-card-text>
        <p>
          <strong>Original Price:</strong> {{ currencyIcon }}
          {{ currencyStore.actualPrice(items.price) }}
        </p>
        <p>
          <strong>Discounted Price:</strong> {{ currencyIcon }}
          {{
            currencyStore.convertPrice(items.price, items.discountPercentage)
          }}
        </p>
      ...
      <v-card-actions>
        ...
        <v-btn v-else color="primary" @click="cartStore.addCart(items)"
          >Add to Cart</v-btn
        >
      </v-card-actions>
    </v-card>
